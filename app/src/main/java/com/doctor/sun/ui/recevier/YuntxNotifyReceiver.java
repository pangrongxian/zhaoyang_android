...
public class YuntxNotifyReceiver extends ECNotifyReceiver {
   ...
   //Log.d("ECSDK_Demo.YuntxNotifyReceiver", "onNotificationClicked notifyType " + notifyType + " ,sender " + sender);
           ...
           //Log.e(TAG , "dispatch receive message fail.");
            ...
            switch (subOptCode){
                case OPTION_SUB_NORMAL:
                    ...
                    OnMessageReceiveListener.getInstance().OnReceivedMessage(message);
                    break;
                case OPTION_SUB_GROUP:
                    ECGroupNoticeMessage notice = intent.getParcelableExtra(EXTRA_MESSAGE);
                    OnMessageReceiveListener.getInstance().OnReceiveGroupNoticeMessage(notice);
                    break;
                case OPTION_SUB_MESSAGE_NOTIFY:
                    ECMessageNotify notify = intent.getParcelableExtra(EXTRA_MESSAGE);
                    OnMessageReceiveListener.getInstance().onReceiveMessageNotify(notify);
                    break;
            }
        }
    }
